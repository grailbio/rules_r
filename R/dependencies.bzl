...
load(
    "@rules_r//internal:versions.bzl",
    _is_at_least = "is_at_least",
)
load(
    "@rules_r//R/makevars:darwin.bzl",
    _local_darwin_makevars = "local_darwin_makevars",
)
load(
    "@rules_r//R/makevars:linux.bzl",
    _local_linux_makevars = "local_linux_makevars",
)
load(
    "@rules_r//R/internal/toolchains:local_toolchain.bzl",
    _local_r_toolchain = "local_r_toolchain",
)
load(
    "@rules_r//R/internal:coverage_deps.bzl",
...
def r_rules_dependencies():
    ...
    _maybe(
        ...
        name = "rules_r_makevars_darwin",
    )
    _maybe(
        _local_linux_makevars,
        name = "rules_r_makevars_linux",
    )

def r_register_toolchains(**kwargs):
    _maybe(
        _local_r_toolchain,
        name = "rules_r_toolchains",
        **kwargs
    )

    native.register_toolchains(
        "@rules_r_toolchains//:toolchain",
    )
