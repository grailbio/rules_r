...
load("@com_rules_r//internal:os.bzl", _detect_os = "detect_os")
...
_BUILD = """load("@com_rules_r//R/internal/toolchains:toolchain.bzl", "r_toolchain")
...
toolchain(
    name = "toolchain",
    toolchain = "@com_rules_r_toolchains//:r_toolchain_generic",
    toolchain_type = "@com_rules_r//R:toolchain_type",
    visibility = ["//visibility:public"],
)
"""
...
def _local_r_toolchain_impl(rctx):
    ...
    if rctx.attr.makevars_site:
        makevars_repo = "@com_rules_r_makevars_%s" % os
...
local_r_toolchain = repository_rule(
    attrs = {
        ...
        "_system_state_computer": attr.label(
            ...
            default = "@com_rules_r//R/scripts:system_state.sh",
